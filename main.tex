...
Figure~\ref{fig:e1} gives the result, with full numbers in
Table~\ref{tab:e1}. Fine-tuning on one harness raised accuracy on that harness
...
from, but less than the noise floor under \codexcli{} ($68.0\%$ before
fine-tuning, $66.7\%$ after), so its spread across harnesses grows from $3.4$
to $11.7$ points. \method{} improves on the base model by $7.6$ points on
...
is level with the control on Verified ($78.6\%$ against $78.4\%$, within the
noise floor) and ahead on Multilingual ($75.0\%$ against $68.7\%$), although
it never trained on \claudecode{}'s own surface. Under the three unseen
...
\chr{} renders for training (Appendix~\ref{sec:limits}). The advantage is
therefore largest where the control is weakest: \method{}'s worst harness is
...
reach an unseen surface, and we do not have an explanation for it. An earlier
run of the same comparison on a weaker base model with a different recipe is
reported in Appendix~\ref{app:e1-30b}; there \method{}'s advantage appeared on
Multilingual only.
...
materially determines performance, and recent measurements find that the
...
normalization, factored into axes that are sampled, and proved invertible.
...
production harnesses reduce to $12$ semantic actions, and the surface map
between them is invertible on real actions. Making that core explicit as the
\HP{} turns interface variation into a nuisance factor that a training
pipeline can marginalize over, and \method{} does so with no new task data
and no architectural change: at an equal number of training steps it matches
a single-harness control on that control's harness, is ahead under every
unseen production harness by up to $8.7$ points, and performs on a par with
training on real trajectories collected in four harnesses. Aligning hidden

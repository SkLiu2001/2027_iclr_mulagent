...
a repository, editing files and running tests. A model does this through a
\emph{harness} such as \claudecode{}, \codexcli{}, \opencode{} or
\openhands{}: the scaffold that decides which tools it sees, what they are
called, and how calls and results are formatted. These details differ across
harnesses and they matter. With model and tasks fixed, changing only the
harness moves \swev{} accuracy by up to $9.0$ points, and fine-tuning on
trajectories from one harness teaches the interface along with the task: the
gain is largest on the source harness and absent on another, widening the
spread across harnesses from $3.4$ to $11.7$ points. Examining \numactions{}
real tool calls from four production harnesses, we find that they all reduce
to the same $12$ semantic actions. We formalize this shared core as the
\HP{} and use it to separate a harness into two layers: the protocol
trajectory, the actions and observations every harness must carry, and a
\emph{surface} of tool names, documentation, output and turn formatting.
Rendering a protocol trajectory into any surface and parsing it back recovers
the same actions (\numroundtrip{} checks, no mismatches). \method{}
(Harness-INvariant Training) exploits this: it parses each real trajectory
into the protocol and re-renders it under many sampled surfaces, some with
meaningless tool names, so that one harness's data trains an agent for any
harness with no new task data. Fine-tuning \basemodel{} on \claudecode{}
trajectories, \method{} matches a step-matched control on \claudecode{}
itself, leads under every unseen production harness by up to $8.7$ points,
narrows the \swem{} accuracy range across five harnesses from $7.4$ to $1.6$
points, and is on a par with training on real trajectories from four
harnesses.
\end{abstract}
...
LLM agents now resolve real software issues end to end. Given an issue
against a GitHub repository, an agent explores the codebase, edits files,
runs tests and returns a patch, and SWE-bench \citep{jimenez2024swebench}
measures how often that patch is correct. The model never acts on the
repository directly: it acts through a \emph{harness}, the scaffold that
documents a tool set in the system prompt, defines how calls are written,
executes them and formats what comes back. Several harnesses are in
production use, among them \claudecode{} \citep{anthropic2025claudecode},
\codexcli{} \citep{openai2025codex}, \opencode{} \citep{opencode2025} and
\openhands{} \citep{wang2025openhandssdk}, and a model is expected to work in
any of them.

The same model, however, does not perform the same across harnesses, and
fine-tuning widens the gap. With model and tasks fixed, changing only the
harness moves \swev{} accuracy by up to $9.0$ points (Table~\ref{tab:e0}).
Fine-tuning then binds the model to one harness: trajectories are collected
in whichever harness the data pipeline uses, and supervised fine-tuning (SFT)
on them teaches the task and that harness's interface at once. Fine-tuning
\basemodel{} on \claudecode{} trajectories helps it most under \claudecode{}
and not at all under \codexcli{}, widening its spread across harnesses from
$3.4$ to $11.7$ points (Figure~\ref{fig:overview}, left). Since training data
come from one harness but models are deployed in many, every new harness
would otherwise call for new trajectories.

\begin{figure}[t]
\begin{center}
\includegraphics[width=\textwidth]{pic/intro_v1.pdf}
\end{center}
\caption{\textbf{Left:} standard training binds the policy to the harness its
trajectories were collected in, and moving the model to a harness
...
core all harnesses share, and re-renders it into many counterfactual
harnesses whose surface axes are independently resampled, training an agent
invariant to the surface rather than bound to it.}
\label{fig:overview}
\end{figure}

We argue that this shift can be avoided by separating what a harness asks the
agent to do from how it presents it. Harnesses disagree on presentation
almost everywhere: reading a file is \texttt{Read} in \claudecode{},
\texttt{read} in \opencode{}, \texttt{file\_editor} with
\texttt{command="view"} in \openhands{}, and a \texttt{sed -n} command in
\codexcli{}, which has no file tools at all. Underneath, they do the same
things: the four harnesses declare between $4$ and $21$ tools, yet all but
$0.04\%$ of \numactions{} real calls map onto the same $12$ semantic actions
(Table~\ref{tab:vocab}). We make this shared core explicit as the \HP{} and
split every harness into two layers (Section~\ref{sec:protocol}): a
\emph{semantic} layer, the protocol trajectory of actions and observations
that any harness must carry, and a \emph{surface} layer of five independently
settable axes, namely tool granularity and naming, call syntax, prompt
documentation, observation formatting and turn conventions. The split loses
nothing: rendering a protocol trajectory into any surface and parsing it back
recovers the same actions in all \numroundtrip{} render--parse checks.

Because the two layers separate exactly, one collected trajectory stands for
a whole family of trajectories that differ only in surface. We introduce
\method{} (Harness-INvariant Training), which trains on that family
(Figure~\ref{fig:overview}, right). Counterfactual harness rendering
(\chr{}) parses each real trajectory into the protocol and re-renders it under
freshly sampled surfaces, and interface anonymization replaces tool names
with meaningless identifiers in a fraction of samples, so that the model must
read the documentation rather than recognize a name. \method{} changes only
the data: it needs no new tasks and no change to the model or objective.

We fine-tune \basemodel{} on \claudecode{} trajectories and compare \method{}
with a control trained on the same trajectories in their original harness
for the same number of optimizer steps. \method{} matches the control on
\claudecode{} itself, leads under each of the three unseen production
harnesses by up to $8.7$ points, and narrows the accuracy range across five
harnesses on \swem{} from $7.4$ to $1.6$ points
(Section~\ref{sec:step-matched}). It also performs on a par with a model
trained on real trajectories collected in four harnesses
(Section~\ref{sec:real}). Our contributions are:
\begin{itemize}
\item \textbf{A two-layer view of harnesses}: four production harnesses share
  one semantic core, which we formalize as the \HP{}, and surfaces can be
  removed and re-applied without changing actions (Section~\ref{sec:protocol}).
\item \textbf{\method{}}, a data-only training method that turns trajectories
  from one harness into training data for many (Section~\ref{sec:method}).
\item \textbf{Evidence} that harness choice alone moves accuracy, that
  \method{} reduces this dependence at an equal training budget, and analyses
  of the surface cues models rely on (Section~\ref{sec:exp}).
\end{itemize}
...
Reading a file is a \texttt{Read} call in \claudecode{}, \texttt{read} in
\opencode{}, \texttt{file\_editor} with \texttt{command="view"} in
\openhands{}, and \texttt{sed -n} or \texttt{cat} under \codexcli{}; the
protocol records all four as one action, \texttt{read\_file}, and treats the rest as a property of the harness.

\paragraph{Semantic actions.} Let $\mathcal{A}$ be a finite vocabulary of
semantic actions, each a verb with a typed argument record. We take
...
\numactions{} real calls fall outside it. A \emph{protocol
...
\begin{table}[b]
\caption{The $12$-action vocabulary from normalizing four harnesses, with its
share of \numactions{} real calls; \texttt{unknown} is the
residual outside the vocabulary. Percentages are rounded independently.}
\label{tab:vocab}
\begin{center}
\begin{small}
\begin{tabular}{lrlr}
...
everything else. ``Tools'' counts the declared schema; granularity is the
$A_1$ setting, where shell only means that files are read, searched and edited
through shell commands. Tool names are those of the parsed releases; the
evaluated releases rename a few of them (Appendix~\ref{app:eval}).}
\label{tab:harnesses}
\begin{center}
\begin{small}
\begin{tabular}{lrlll}
...
the extremes are instructive. \openhands{} exposes $4$ tools; \claudecode{} exposes $21$.
...
and of those commands $27\%$ are file reads (\texttt{sed -n}, \texttt{cat},
\texttt{head}), $18\%$ searches (\texttt{grep}, \texttt{find}) and $10\%$
patch applications. More than half of \codexcli{}'s
shell traffic is, semantically, what other harnesses expose as first-class
tools. Surface variation of this magnitude over an identical semantic core is the
regime in which a surface-bound policy fails and an
...
sampled surface configurations (four per action for the mixed set, three
otherwise) and parsed back; exact means that the verb and all arguments
match, $\simeq$ equality up to the operational equivalence of
Proposition~\ref{prop:roundtrip}, which arises only in the shell family. The
$232$ calls outside the vocabulary are not rendered.}
\label{tab:roundtrip}
\begin{center}
\begin{small}
...
Section~\ref{sec:exp} measures deviation from this equality, and \method{} is
a training method that pursues it, whence its name. \method{} has
...
only the data; the model architecture and training objective are unchanged. Figure~\ref{fig:pipeline} traces the pipeline end to end.
Because real harnesses cannot be perturbed one axis at a time, we also
implement \cham{}, an executable harness in which every surface property is a
configuration parameter drawn from the five axes of Eq.~(\ref{eq:axes}); it
parses calls, executes them in a real workspace and formats observations
according to its configuration, serving as the instrument for single-axis
experiments (Section~\ref{sec:axes}) and as an evaluation environment under
synthetic surfaces.
...
Every sample keeps the action sequence and the observation content of a
real, verified episode; only their rendering is synthetic, and training
renderings never truncate observations. What a rendering does not carry is
the executor behind a surface, such as a tool's preconditions or the calls a
harness rejects; Appendix~\ref{app:iface-errors} measures these as interface
errors on the real harnesses. Rendering is domain randomization
...
learning, that augmentation perturbs \emph{style} and leaves \emph{content}
untouched \citep{kugelgen2021self}, holds on the action channel by
Proposition~\ref{prop:roundtrip}.
...
anonymization component: every tool name in such a sample is replaced, while
the documentation, which is never anonymized, becomes the only channel
...
as teacher on SWE-rebench-V2 instances \citep{badertdinov2026swerebenchv2},
one per instance, deduplicated against both evaluation benchmarks. The
control trains on these trajectories as collected, and \method{} on their
\chr{} renderings alone: $M=3$ counterfactual surfaces per trajectory,
\numsurfaces{} samples in all with $21{,}476$ distinct tool names, $30\%$ of
them anonymized, all in native function calling (surface statistics in
Appendix~\ref{app:eval}). Both runs are full-parameter fine-tuning with a
global batch of $128$ packed sequences of up to $256$K tokens, learning rate
$5\times10^{-5}$ and cosine decay.

\paragraph{Step-matched control.} Rendering multiplies the data: the
rendered set holds about $1.7\times$ the original tokens, so a gain could come
from extra optimization rather than surface diversity. We therefore match optimizer steps: the \control{} trains on the
original \claudecode{} trajectories for three epochs, $67$ steps, and
\method{} for the same $67$ steps, about $1.8$ passes over its rendered set;
with sequence packing, equal steps at equal batch size are also
approximately equal training tokens. The two runs differ only in their data:
the control sees each trajectory three times in its original surface,
\method{} in three different surfaces, none of them \claudecode{}'s own.

\paragraph{Evaluation.} We report pass@1 on \swev{}
\citep{jimenez2024swebench}, $500$ human-validated tasks from $12$ Python
repositories, and on \swem{} \citep{yang2025swesmith}, $300$ tasks in nine
other programming languages; we call them Verified and Multilingual. Every
model runs under the four production harnesses of Table~\ref{tab:harnesses}
and under \cham{} with one fixed configuration, served with vLLM
\citep{kwon2023vllm} on one serving stack. Section~\ref{sec:harness-choice}
evaluates Qwen3.5-35B-A3B \citep{qwen35card}, \basemodel{} and
KAT-Coder-V2.5-Dev \citep{huang2026katcoder} once per cell,
Section~\ref{sec:step-matched} averages three evaluations per cell, and the
later experiments state their own conditions. We call \opencode{},
\openhands{} and \codexcli{} unseen because training never presented any of
them as a coherent harness (Appendix~\ref{sec:limits} counts the rendered
samples that carry one of their tool names). Appendix~\ref{app:eval} summarizes the settings of every experiment in
...
models on \swev{} (V, $500$ tasks) and \swem{} (M, $300$ tasks) under four
production harnesses and \cham{} (fixed configuration), one evaluation per cell. Spread:
max minus min.}
\label{tab:e0}
\begin{center}
\begin{small}
...
Table~\ref{tab:e0} establishes the phenomenon. The ordering
...
of the four production harnesses on Verified for all three models, while
\openhands{}, with $4$ tools, is the best or second best for the two Qwen
models. The size of the spread is a property of the model: $9.0$
...
$7.0$, $3.0$). KAT-Coder-V2.5-Dev, post-trained with harness randomization
\citep{huang2026katcoder}, shows the smallest spread; \method{} aims to obtain
the same property from training data alone.

\subsection{\method{} versus a step-matched control}
\label{sec:step-matched}

\begin{figure}[t]
\begin{center}\includegraphics[width=0.85\textwidth]{pic/e1_step_matched.pdf}
\end{center}
\caption{\method{} versus the \control{}, both fine-tuned from \basemodel{}:
pass@1 (\%) on \swev{} and \swem{} under each harness, the fine-tuned models
averaged over three evaluations. Each row places the base model (open
diamond), the control (grey square) and \method{} (orange circle); the bar
joins the two fine-tuned models, and the number to its right is \method{} minus control. Full numbers, with the worst harness and
the spread, are in Table~\ref{tab:e1}.}
\label{fig:e1}
\end{figure}

Figure~\ref{fig:e1} gives the result (full numbers in Table~\ref{tab:e1}). Fine-tuning on one harness raised accuracy on that harness
and made the model more dependent on it: on Verified the control gains $9.4$
points over the base model under \claudecode{}, the harness its data came
from, but none under \codexcli{}, so its spread across harnesses grows from $3.4$ to $11.7$ points. \method{} improves on the base model by $7.6$ points on
Verified and $10.3$ on Multilingual on average, against the control's $5.0$
and $6.6$, and its gain is spread across harnesses. On the source harness it
is level with the control on Verified ($78.6\%$ against $78.4\%$) and ahead
on Multilingual ($75.0\%$ against $68.7\%$), although it never trained on
\claudecode{}'s own surface. Under the three unseen
production harnesses it is ahead on both benchmarks, by $2.8$, $4.5$ and $8.7$
points on Verified and by $1.5$, $2.3$ and $7.4$ on Multilingual; the largest
gap is under \codexcli{}, whose shell-only surface lies outside the surfaces
\chr{} renders for training (Appendix~\ref{sec:limits}). The advantage is largest where the control is weakest: \method{}'s worst harness is
$8.7$ points (Verified) and $7.4$ points (Multilingual) above the control's,
and on Multilingual its five accuracies lie within $1.6$ points
($73.4$--$75.0\%$), a tighter range than the base model's own $7.0$. \method{} is ahead in eight of the ten cells by at least one point, within a
point of the control in one (\claudecode{} on Verified), and behind in one.

That one cell is \cham{} on Verified, where the control leads by $2.8$
points; \cham{}'s configuration comes from the space \method{}'s training
surfaces were sampled from, so this is not a failure to reach an unseen
surface, and we have no explanation for it. An earlier run on a weaker base model with a different recipe
(Appendix~\ref{app:e1-30b}) showed \method{}'s advantage on Multilingual only.
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=0.85\textwidth]{pic/real_harness_data.pdf}
\end{center}
\caption{\method{} versus training on real trajectories from four harnesses,
both fine-tuned from \oldmodel{} for equal steps,
under the four production harnesses and \cham{} (pass@1, one
...
trajectories in several harnesses. We compare the two on \oldmodel{}, the base
model of Appendix~\ref{app:e1-30b}: the \method{} model of that appendix
trains on \claudecode{} trajectories mixed with their \chr{} renderings, and the alternative trains on real trajectories collected in all four
production harnesses with the same schedule and number of optimizer steps. Four of the five evaluation surfaces thus occur in
the alternative's training data, whereas for \method{} every surface but
\claudecode{} is unseen. As Figure~\ref{fig:real} shows, \method{} is nonetheless ahead under \claudecode{}, by $16.4$ and $14.7$
points on Verified and Multilingual, and under \opencode{}, by $3.8$ and
$10.3$ points, although the other model trained on \opencode{} trajectories,
and within a few points of it under the other three surfaces. Averaged over the five harnesses it is $2.1$ points
higher on Verified and $4.7$ on Multilingual, and its accuracy varies across
harnesses by $16.9$ and $19.3$ points against the other model's $26.6$ and
$24.0$. At an equal step budget, then, synthetic surfaces perform on a par with real
trajectories distilled in the target harnesses. The two training sets also differ in their tasks, so
the comparison is between two data pipelines rather than between surfaces
alone.
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=0.75\textwidth]{pic/anon_names.pdf}
\end{center}
\caption{Anonymizing tool and parameter names under \cham{} on \swem{}, for
\basemodel{} before fine-tuning and the \method{} model of
Section~\ref{sec:step-matched}, one evaluation per setting. \textbf{(a)}~Pass@1 as names are replaced. \textbf{(b)}~Tasks that flip with
all names replaced.}
\label{fig:anon}
\end{figure}

Whole harnesses differ from the training surfaces in many respects at once, so
they cannot show which cue a model relies on. We isolate the cheapest cue, the identifiers of tools and parameters, which
let a model that recognizes \texttt{Bash} act without reading its description. Holding the
\cham{} configuration of Table~\ref{tab:e0} fixed ($9$ tools, $17$
parameters), we replace about half of the names, then all of them, with
meaningless identifiers (\texttt{run\_command} becomes \texttt{zete}). Tool descriptions are identical across settings and the system prompt names
no tool, so what each tool does stays readable. \basemodel{} before
fine-tuning and the \method{} model of Section~\ref{sec:step-matched} are
evaluated once per setting on \swem{}, each compared with its own run under
the original names. As Figure~\ref{fig:anon} shows, \method{}'s
...
and gaining $17$, and its new failures coincide with the new identifiers: calls to tools that
do not exist rise from $5$ to $27$ of the $300$ tasks, misspelling the new
identifiers (\texttt{kazu\_op} for \texttt{kozu\_op}) rather than reverting
to the original ones. \method{}'s accuracy thus does not depend on recognizable identifiers, the
behaviour the anonymized tier is meant to teach, although the $22$ flips in
each direction show that individual episodes still change. The test is in
distribution, since training used the same identifier generator, so it shows
that the behaviour was acquired, not that it extends to unseen naming
schemes.
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=0.75\textwidth]{pic/chameleon_axes.pdf}
\end{center}
...
Production harnesses differ on several axes at once, so
Section~\ref{sec:step-matched} cannot say which axis a model is bound to;
\cham{} can move them one at a time. From an origin configuration that mimics \claudecode{}, we move each of A1,
A3, A4 and A5 in turn to a far setting (Appendix~\ref{app:eval} lists them), and two further
surfaces change A1 and A5 together and all four axes. A2 stays native function calling, and every surface exposes the same
actions. We evaluate the
\method{} model of Appendix~\ref{app:e1-30b} and a control fine-tuned from
\oldmodel{} on the same \claudecode{} trajectories alone, once per surface on \swem{}, in an environment that differs from that
appendix, so only differences between surfaces are interpreted. The control is bound to A1 (Figure~\ref{fig:axes}a): changing
naming and granularity alone takes it from $15.1\%$ on the origin to $3.3\%$,
and it stays there whenever A1 is changed, whereas changing A3, A4 or A5 alone
costs it at most $2.9$ points. \method{} is not: with A1 changed alone it
scores $28.3\%$ against $24.0\%$ on the origin, and it is ahead of the control
on every surface, by $8.9$ points on the origin and by up to $29.9$
(Figure~\ref{fig:axes}b); only the surface that changes all four axes brings
it down, to $17.0\%$. A model trained on one harness has thus absorbed its tool vocabulary and
granularity as part of the task; \method{} has not, and loses only when every
axis changes at once.

\section{Related work}
\label{sec:related}

\citet{yang2024sweagent} showed that the agent--computer interface materially
determines performance, and recent measurements find that the
choice of scaffold moves scores by as much as the choice of model
\citep{zhang2026stopcomparing,yao2026harnessbench,zheng2026clawswebench}.
Data pipelines such as SWE-Gym \citep{pan2024swegym}, SWE-smith
\citep{yang2025swesmith} and SWE-rebench \citep{badertdinov2026swerebenchv2}
scale tasks and trajectories within one scaffold, while a growing line of
work trains across many, by mixing real trajectories from several scaffolds
\citep{abadji2026laguna} or randomizing the harness during reinforcement
learning \citep{huang2026katcoder,xu2026polar}; to our knowledge, we are the first to synthesize surfaces by re-rendering
one harness's real trajectories (Section~\ref{sec:real} compares the two).
\citet{gu2026interfaces} show that trajectory SFT amplifies interface
memorization over semantic understanding, which \method{} counters by domain
randomization \citep{tobin2017domain} over an explicit, invertible surface
space rather than by a penalty as in invariant risk minimization
\citep{arjovsky2019irm}. The Agent Data Protocol \citep{song2025adp} also
defines a scaffold-independent trajectory representation, compiled into fixed
target formats; ours is recovered from production harnesses by
normalization, factored into sampled axes, and proved invertible.
Appendix~\ref{app:related} discusses further related work.

\section{Conclusion}

Agent harnesses differ far more on the surface than in substance: four
production harnesses reduce to $12$ semantic actions, and making that core
explicit as the \HP{} turns interface variation into a nuisance factor that a training
pipeline can marginalize over, and \method{} does so with no new task data
and no architectural change: it matches a single-harness control on that
control's harness, is ahead under every unseen
production harness, and is on a par with
training on real trajectories from four harnesses. Aligning hidden
representations across renderings is the natural next step.
Appendix~\ref{sec:limits} discusses the limitations of this study.
...
evaluation counts and conditions (Table~\ref{tab:settings}), the rounding rule and the harness releases evaluated. We plan to release the parsers, the renderer, the \cham{} harness,
...
decoration, and may truncate long outputs to a character budget when a configuration
sets one. The training sampler never sets one, so training renderings keep
observation content whole; truncation occurs only in the evaluation surface
of Section~\ref{sec:axes} that imitates production behaviour. Proposition~1
is in any case a statement about the action channel.
...
Table~\ref{tab:e1-30b} gives the result. Two differences stand out, both on Multilingual: $2.0$ points on \claudecode{} and $7.7$ points in
...
\paragraph{Training-surface statistics.} Of the \numsurfaces{} rendered
samples of Section~\ref{sec:setup}, $10{,}335$ use split tools, $10{,}511$ a
unified editor and $5{,}185$ a monolithic tool; the samples vary $A_1$, $A_3$,
$A_4$ and $A_5$ and fix $A_2$ to native function calling; a trajectory that
contains a call outside the vocabulary (the \texttt{unknown} residual of
Table~\ref{tab:vocab}) or fewer than three actions yields no rendering.
Normalization moves every call out of the text channels, and we check that no call text remains
in the reasoning or observations of any kept trajectory.

\paragraph{Full numbers of the step-matched comparison.}
Table~\ref{tab:e1} lists the cells behind Figure~\ref{fig:e1}.

\begin{table}[ht]
\caption{Full numbers behind Figure~\ref{fig:e1}: \method{} versus the
\control{}, both fine-tuned from \basemodel{}, pass@1 (\%) on \swev{} and
\swem{}. Fine-tuned cells are means of three evaluations; Base is the model
before fine-tuning (Table~\ref{tab:e0}). $\Delta$ is \method{} minus control,
and bold marks the better of the two (the lower, for spread). Mean, worst and
spread are taken over the five harnesses.}
\label{tab:e1}
\begin{center}
\begin{small}
\setlength{\tabcolsep}{4.5pt}
...
\paragraph{Surfaces of Section~\ref{sec:axes}.} The origin mimics
\claudecode{}: split tools with its names and parameters, an engineering
system prompt with formal tool documentation, numbered file views and
\claudecode{}'s turn conventions. The far settings are: A1 merges the tools
into a single monolithic tool with anonymized tool and parameter names; A3
changes the persona of the system prompt and renders the tool documentation
as tables; A4 drops line numbers, wraps observations in a fence, truncates
them at $2{,}000$ characters and changes the markers for empty and failed
output; A5 removes the runtime reminder, adds a turn header, returns
observations as user turns and ends an episode with a finish call. These runs
exclude the few episodes still running when a cell was closed (at most $15$
of $300$), so their absolute accuracies are not comparable with
Table~\ref{tab:e1-30b}.
...
fine-tuned model is a single training run. Because \chr{}'s first naming tier
...
effect scales with model size or holds for a shell-only source harness
remains open.

...
\newcommand{\hdr}[2]{\begin{tabular}[b]{@{}r@{}}#1\\#2\end{tabular}}  % two-line column header
...
\item \textbf{A step-matched evaluation.} Against a control trained on the
  ...
  $1.6$ points (Section~\ref{sec:exp}).
\end{itemize}
...
to $256$K tokens, learning rate $5\times10^{-5}$ with cosine decay, and $67$
optimizer steps.
...
\paragraph{Evaluation.} We report \swev{} ($500$ tasks) and \swem{} ($300$
tasks) under the four production harnesses of Table~\ref{tab:harnesses} and
under \cham{} with one fixed configuration, with all models served by vLLM on
one serving stack. For the models of E1, \claudecode{} is the source harness;
\opencode{}, \openhands{} and \codexcli{} are unseen as harnesses, though not
all as vocabularies: \chr{}'s name pool excludes \openhands{}'s tool names,
while \opencode{}'s occur, recombined with others, in $33\%$ of rendered
samples and \codexcli{}'s in $21\%$ (Section~\ref{sec:limits}). \cham{}'s
configuration is drawn from the space \chr{} samples from: it is new to both
models, but of a kind \method{} was trained on. E0 additionally runs three
open-weight models, Qwen3.5-35B-A3B, Qwen3.6-35B-A3B and KAT-Coder-V2.5-Dev.
Each E1 cell is the mean of three evaluations; E0 cells are single evaluations.
The task images retain the repository's later git history, and an agent can
download the upstream patch when the sandbox has network access; in E0, an
episode in which the agent obtained the reference fix in either way is scored
as a failure whatever the verifier says (at most $3.2$ points of a Verified
cell and $9.3$ points of a Multilingual cell). \todo{apply the same audit to
the E1 cells} Repeated submissions of an identical configuration give a
single-evaluation noise floor of $\pm2.0$ points on the $500$-task set and
$\pm1.0$ points on the $300$-task set; differences below these are not
interpreted.

\subsection{E0: harness choice alone moves accuracy}

\begin{table}[t]
\caption{Harness choice alone moves accuracy. Three open-weight models under
the four production harnesses and \cham{} (one fixed configuration): pass@1
(\%) on \swev{} (V, $500$ tasks) and \swem{} (M, $300$ tasks), one evaluation
per cell. Spread is max minus min over the five harnesses. Qwen3.6-35B-A3B is
also the base model of E1.}
\label{tab:e0}
\begin{center}
\begin{footnotesize}
...
\toprule
Model & & \hdr{Claude}{Code} & \hdr{Open-}{Code} & \hdr{OpenHands}{SDK} & \hdr{Codex}{CLI} & \hdr{}{\cham{}} & Spread \\
\midrule
Qwen3.5-35B-A3B    & V & $64.2$ & $66.4$ & $67.6$ & $58.6$ & $67.2$ & $9.0$ \\
                   & M & $54.7$ & $54.3$ & $58.3$ & $49.7$ & $55.0$ & $8.6$ \\
Qwen3.6-35B-A3B    & V & $69.0$ & $71.4$ & $71.0$ & $68.0$ & $70.8$ & $3.4$ \\
                   & M & $62.3$ & $68.3$ & $64.3$ & $61.3$ & $62.3$ & $7.0$ \\
KAT-Coder-V2.5-Dev & V & $69.4$ & $69.8$ & $68.8$ & $68.6$ & $67.2$ & $2.6$ \\
                   & M & $57.3$ & $58.7$ & $60.3$ & $59.3$ & $60.0$ & $3.0$ \\
\bottomrule
\end{tabular}
\end{footnotesize}
\end{center}
...
\subsection{E1: \method{} versus a step-matched control}

\begin{table}[t]
\caption{\method{} versus the \control{}, both fine-tuned from \basemodel{}
for $67$ steps: pass@1 (\%) under the source harness, three unseen production
harnesses and \cham{}. Fine-tuned cells are means of three evaluations; the
base row is the single E0 evaluation. $\Delta$ is \method{} minus control.
Worst is the lowest of the five harnesses and spread is max minus min. Bold
marks the better fine-tuned model (the lower, for spread). The
single-evaluation noise floor is $\pm2.0$ (V) and $\pm1.0$ (M).}
\label{tab:e1}
\begin{center}
\begin{footnotesize}
...
\toprule
 & & \hdr{Claude Code}{(source)} & \hdr{Open-}{Code} & \hdr{OpenHands}{SDK} & \hdr{Codex}{CLI} & \hdr{}{\cham{}} & Mean & Worst & Spread \\
\midrule
V & Base      & $69.0$ & $71.4$ & $71.0$ & $68.0$ & $70.8$ & $70.0$ & $68.0$ & $3.4$ \\
  & Control   & $78.4$ & $74.3$ & $77.2$ & $66.7$ & $\mathbf{78.4}$ & $75.0$ & $66.7$ & $11.7$ \\
  & \method{} & $\mathbf{78.6}$ & $\mathbf{77.1}$ & $\mathbf{81.7}$ & $\mathbf{75.4}$ & $75.6$ & $\mathbf{77.7}$ & $\mathbf{75.4}$ & $\mathbf{6.3}$ \\
  & $\Delta$  & $+0.2$ & $+2.8$ & $+4.5$ & $+8.7$ & $-2.8$ & $+2.7$ & $+8.7$ & $-5.4$ \\
\midrule
M & Base      & $62.3$ & $68.3$ & $64.3$ & $61.3$ & $62.3$ & $63.7$ & $61.3$ & $7.0$ \\
  & Control   & $68.7$ & $71.9$ & $71.7$ & $66.0$ & $73.4$ & $70.3$ & $66.0$ & $7.4$ \\
  & \method{} & $\mathbf{75.0}$ & $\mathbf{73.4}$ & $\mathbf{74.0}$ & $\mathbf{73.4}$ & $\mathbf{74.4}$ & $\mathbf{74.0}$ & $\mathbf{73.4}$ & $\mathbf{1.6}$ \\
  & $\Delta$  & $+6.3$ & $+1.5$ & $+2.3$ & $+7.4$ & $+1.0$ & $+3.7$ & $+7.4$ & $-5.8$ \\
\bottomrule
\end{tabular}
\end{footnotesize}
\end{center}
...
own $7.0$. Against the single-evaluation noise floor, \method{} is ahead in
...
The step-matched control rules out ``more gradient steps''. It does not
...
$8{,}677$ trajectories into the same number of samples at the same token count
...
\paragraph{Strength of evidence.} Each E1 cell averages three evaluations, but
...
($\pm2.0$ points, Section~\ref{sec:exp}); the remainder is what the interface,
...
Before the experiment of Section~\ref{sec:exp} we ran the same comparison at a
...
access, and without the leak audit of Section~\ref{sec:exp}.
...
\toprule
Harness & Benchmark & Base & Control & \method{} & $\Delta$ \\
\midrule
\claudecode{} (source) & Verified     & $25.2$ & $47.8$ & $48.2$ & $+0.4$ \\
\claudecode{} (source) & Multilingual & $14.7$ & $33.7$ & $35.7$ & $+2.0$ \\
\opencode{} (unseen)   & Verified     & $16.0$ & $43.0$ & $42.0$ & $-1.0$ \\
\opencode{} (unseen)   & Multilingual & $13.3$ & $23.3$ & $\mathbf{31.0}$ & $\mathbf{+7.7}$ \\
\bottomrule
\end{tabular}
\end{small}
\end{center}

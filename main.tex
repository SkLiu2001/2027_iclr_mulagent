...
with its share of \numactions{} real tool calls; \texttt{unknown} is the
...
parses calls, executes them against a real workspace and formats observations
according to its configuration, and serves as the instrument for single-axis
...
one per task instance and deduplicated against both evaluation benchmarks.
...
(Appendix~\ref{app:eval} gives the surface statistics). \method{} trains on
these rendered samples alone. Both runs are full-parameter fine-tuning with a
global batch of $128$ packed sequences of up to $256$K tokens and learning
rate $5\times10^{-5}$ with cosine decay.
...
with sequence packing, equal steps at an equal batch size are also
...
\method{} in three different surfaces, none of which is \claudecode{}'s own.
...
Section~\ref{sec:step-matched}, one evaluation per setting. \textbf{(a)}~Pass@1 as more names are replaced.

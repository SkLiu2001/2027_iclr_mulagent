...
one per instance, deduplicated against both evaluation benchmarks.
After dropping trajectories with unmapped tools or fewer than three semantic
actions, $8{,}677$ remain; \chr{} renders each into $M=3$ counterfactual
surfaces, yielding \numsurfaces{} samples with $21{,}476$ distinct tool names,
$30\%$ of them anonymized, all in native function calling
(surface statistics in Appendix~\ref{app:eval}). \method{} trains on
these rendered samples alone. Both runs are full-parameter fine-tuning with a
...
samples that carry one of their tool names). Appendix~\ref{app:eval} gives the
...
control, grey when within the single-evaluation noise floor ($\pm2.0$ on
Verified, $\pm1.0$ on Multilingual). Full numbers, with the worst harness and
...
from, but less than the noise floor under \codexcli{}, so its spread across
harnesses grows from $3.4$ to $11.7$ points. \method{} improves on the base model by $7.6$ points on
...
($73.4$--$75.0\%$), a tighter range than the base model's own $7.0$. Against
the single-evaluation noise floor (Appendix~\ref{app:eval}), \method{} is
ahead in seven of the ten cells, level in two (\claudecode{} on Verified,
\cham{} on Multilingual), and behind in one.
...
\method{}'s training surfaces were sampled from, so this is not a failure to reach an unseen surface; we have no explanation
for it. An earlier run on a weaker base model with a different recipe
...
trains on \claudecode{} trajectories mixed with their \chr{} renderings, and
the alternative trains on real trajectories collected in \claudecode{},
\opencode{}, \openhands{} and \codexcli{} with the same schedule and the same
number of optimizer steps. Four of the five evaluation surfaces thus occur in
the alternative's training data, whereas for \method{} every surface but
\claudecode{} is unseen. As Figure~\ref{fig:real} shows, \method{} is
nonetheless ahead under \claudecode{} ($48.2\%$ against $31.8\%$ on Verified,
$35.7\%$ against $21.0\%$ on Multilingual) and under \opencode{}, by $3.8$ and
$10.3$ points, although the other model trained on \opencode{} trajectories,
and it is within a few points of the other model under \openhands{},
\codexcli{} and \cham{}. Averaged over the five harnesses it is $2.1$ points
...
$24.0$. At an equal step budget, then, training data whose surfaces are
synthetic performs on a par with real trajectories distilled in the target
harnesses themselves. The two training sets also differ in their tasks, so
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=0.8\textwidth]{pic/anon_names.pdf}
\end{center}
...
meaningless identifiers (\texttt{run\_command} becomes \texttt{zete}). The tool
...
\texttt{kozu\_op}) rather than revert to the original ones. \method{}'s accuracy thus does not depend on recognizable identifiers, the
...
\begin{figure}[t]
\begin{center}
\includegraphics[width=0.85\textwidth]{pic/chameleon_axes.pdf}
\end{center}
...
a time. From an origin configuration that mimics \claudecode{}, we move each of A1,
A3, A4 and A5 in turn to a far setting (a monolithic tool with anonymized
names, a different persona with tabular documentation, unnumbered fenced
observations, and observations as user turns; Appendix~\ref{app:eval}), and
two further surfaces change A1 and A5 together and all four axes. The call syntax (A2) is fixed by native
function calling, and every surface exposes the same actions. We evaluate the
\method{} model of Appendix~\ref{app:e1-30b} and a control fine-tuned from
\oldmodel{} on the same \claudecode{} trajectories alone, once per surface on
\swem{}, in a different evaluation environment from that appendix, so only
differences between surfaces are interpreted. The control is bound to A1 (Figure~\ref{fig:axes}a): changing
...
learning \citep{huang2026katcoder,xu2026polar}; we are, to our knowledge, the
first to synthesize surfaces by re-rendering one harness's real trajectories,
and Section~\ref{sec:real} compares the two directly.
...
representations across renderings of the same decision point is the natural
next step.
...
$A_4$ and $A_5$ and fix $A_2$ to native function calling. Normalization moves
every call out of the text channels, and we check that no call text remains

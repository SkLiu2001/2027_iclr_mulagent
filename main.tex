...
original harness for the same number of steps, \method{} matches the control
on that harness, is ahead under every unseen production harness by up to $8.7$
points,
...
$78.4\%$ but moves its \codexcli{} accuracy by less than the noise floor
($68.0\%$ before, $66.7\%$ after), widening its spread across harnesses from $3.4$ to $11.7$
...
re-dressed in an arbitrary surface, then a single collected episode represents an entire family of equivalent
...
of optimizer steps. On the source harness \method{} matches the control on Verified and is ahead
on Multilingual. Under each of the three unseen production harnesses it is ahead on
...
appearance, and the domain-generalization bound of \citet{benda2010theory} motivates it:
the bound charges the divergence between training and deployment domains,
which training over a distribution of surfaces is meant to reduce.
...
samples that carry one of their tool names. Appendix~\ref{app:eval}
...
on \opencode{} trajectories; on \opencode{} Verified the difference is $3.8$ points ($42.0\%$ against
$38.2\%$). Under this data mix and step budget, rendering counterfactual
surfaces gave higher accuracy than adding real trajectories from two more
harnesses, on the source harness and on \opencode{} Multilingual, one of the
harnesses those trajectories came from; the two training sets also differ in
task sets and data quality, so we do not attribute the whole gap to rendering. \todo{extend to \openhands{}, \codexcli{} and \cham{} once
...
\claudecode{} tool name (the source harness's), $33\%$ at least one
\opencode{} name and $21\%$ at least one \codexcli{} name; none carries an
\openhands{} name, because that harness's names are excluded from the pool, and
none presents any of the three held-out harnesses as a coherent harness. \openhands{} is therefore
...
\paragraph{Ablations not run.} We have not separated sampling many surfaces
from changing the surface once, which needs a control trained on one fixed
rendering of the same trajectories, nor trained \chr{} without its anonymized
tier, so the contribution of anonymization is not separated from that of
rendering.
...
augmentation is here measured at \rtacc{}, and the transformations between surfaces are explicit and composable rather
than latent \citep{higgins2018symmetry}. The algebraic reading of the protocol as a normal
...
evaluation counts and conditions (Table~\ref{tab:settings}), the leaked-fix
scoring rule, the rounding rule, the noise floor and the harness releases
evaluated. We plan to release the parsers, the renderer, the \cham{} harness,
...
\subsection{Friction follows the harness more than the model}

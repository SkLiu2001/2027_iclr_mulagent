...
sampling a point is sampling a synthetic harness. Figure~\ref{fig:orbit}
shows one protocol trajectory under three such points.
...
the E1 pair of Section~\ref{sec:exp}, \method{} and its \control{}, on
\claudecode{} (the source harness), \opencode{} and \openhands{}: $4{,}280$
...
harness and on two unseen ones (E1 trajectories). (a)~Calls the harness
...
enters a conclusion (Section~\ref{sec:limits}), but among the episodes that do
not, \method{} follows the convention and the control does not.}
\label{fig:ana-hint}
\end{figure}

Section~\ref{sec:exp} compares \method{} and its \control{} by accuracy. Their
...
Table~\ref{tab:e1} averages over, and they are what interface invariance
...
exist nowhere. The third is not from the E1 arms but from an instruction-tuned

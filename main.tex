...
are anonymized. \method{} trains on these rendered samples alone. Both runs are
...
\midrule
Spread        & 9.0  & 8.6  & 3.4  & 7.0  & 2.6  & 3.0  \\
\bottomrule
\end{tabular}
\end{small}
\end{center}
...
KAT-Coder-V2.5-Dev on Verified, in the same order on Multilingual ($8.6$,
...
\codexcli{}, whose shell-only surface lies outside the granularities \chr{}
renders. The advantage is therefore largest where the control is weakest:
...
reach an unseen surface of the kind the other columns measure, and we do not
...
Production harnesses differ on all surface axes at once, so neither
...
the strictly zero-shot column of Table~\ref{tab:e1}; excluding each held-out
...
recipe on both sides. The data were $5{,}000$ \claudecode{} trajectories, of
which $4{,}133$ survived the filters of Section~\ref{sec:exp}; \chr{} rendered
each into $M=2$ surfaces, giving $8{,}266$ samples under $8{,}204$ distinct
surface configurations with $11{,}205$ distinct tool names. \method{} trained on
the real trajectories \emph{mixed with} their renderings and the control on the
real trajectories alone, each for $215$ optimizer steps at a global batch of
...
Table~\ref{tab:e1-30b} gives the result. The one difference beyond the noise
floor is the unseen-harness, unseen-language cell, where \method{} gains $7.7$
points ($93/300$ against $70/300$; two-proportion $z\approx2.11$,
$p\approx0.035$). Measured as the accuracy each model loses when moved from
\claudecode{} to \opencode{}, \method{} loses $4.7$ points on Multilingual
against the control's $10.4$, but $6.2$ on Verified against the control's

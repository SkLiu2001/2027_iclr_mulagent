...
\method{} has two components, Counterfactual Harness Rendering and interface
anonymization, the second implemented as one tier of the first's name sampler;
both touch only the data, and nothing about the model architecture or the
...
task instance. Normalization moves every call out of the text channels, and we
...
JavaScript, TypeScript, PHP, Ruby and Rust). In
...
vLLM on the same serving stack; Section~\ref{sec:harness-choice} evaluates
three open-weight models, Qwen3.5-35B-A3B, Qwen3.6-35B-A3B and
KAT-Coder-V2.5-Dev, once per cell, and Section~\ref{sec:step-matched}
evaluates the two fine-tuned models and averages three evaluations per cell.
The later experiments state their own conditions. Appendix~\ref{app:eval}
describes how episodes that obtained the reference fix are scored, gives the
run-to-run noise floor, and summarizes the settings of every experiment in
...
declares thirteen tools but routes file operations through a shell, is the lowest
...
harnesses, both fine-tuned from Qwen3-30B-A3B-Instruct-2507 for the same
number of steps, under \claudecode{} and \opencode{} (pass@1, one evaluation
per cell).}
\label{fig:real}
\end{figure}
...
schedule and the same number of optimizer steps. Both models are evaluated under
...
Qwen3-30B-A3B-Instruct-2507, on the same \claudecode{} trajectories alone, once
per surface on \swem{}. These runs use a different evaluation environment from
Appendix~\ref{app:e1-30b}, are not leak-audited, and exclude the few episodes
...
(Appendix~\ref{app:e1-30b}) agrees on Multilingual but not on Verified. All \method{} runs use \claudecode{} as the source harness. Whether the effect
scales with model size, or holds when the source harness is \codexcli{}'s shell-only surface,
remains open.
...
first-class file editors to file access through a shell alone, reduce to twelve semantic
...
endpoint from Section~\ref{sec:setup}, while the sandbox still had network
...
\paragraph{Harness releases.} The releases evaluated here differ from those of
the parsed corpora (Table~\ref{tab:harnesses}) in a few tool names:
...
Section~\ref{sec:setup} and the leaked-fix scoring above.
...
conditions'' is the earlier serving endpoint, with network access in the
sandbox. ``Audited'' refers to the leaked-fix scoring above.}
\label{tab:settings}
\begin{center}
\begin{footnotesize}
...
\midrule
Section~\ref{sec:harness-choice}, Table~\ref{tab:e0} & Qwen3.5-35B-A3B, \basemodel{}, KAT-Coder-V2.5-Dev & not fine-tuned & four production, \cham{} & V, M & 1 & Section~\ref{sec:setup} stack; audited \\
Section~\ref{sec:step-matched}, Table~\ref{tab:e1} & \basemodel{} & \method{}, \control{} & four production, \cham{} & V, M & 3 & Section~\ref{sec:setup} stack; audit pending \\
Section~\ref{sec:real}, Figure~\ref{fig:real} & Qwen3-30B-A3B-Instruct-2507 & \method{} of Appendix~\ref{app:e1-30b}, real-trajectory model & \claudecode{}, \opencode{} & V, M & 1 & Appendix~\ref{app:e1-30b} conditions; not audited \\
Section~\ref{sec:anon}, Figure~\ref{fig:anon} & \basemodel{} & base, \method{} of Table~\ref{tab:e1} & \cham{}, three naming settings & M & 1 & separate runs; not audited \\
Section~\ref{sec:axes}, Figure~\ref{fig:axes} & Qwen3-30B-A3B-Instruct-2507 & \method{} of Appendix~\ref{app:e1-30b}, control & \cham{}, seven surfaces & M & 1 & separate runs; not audited; unfinished episodes excluded \\
Appendix~\ref{app:e1-30b}, Table~\ref{tab:e1-30b} & Qwen3-30B-A3B-Instruct-2507 & \method{}, control & \claudecode{}, \opencode{} & V, M & 1 & Appendix~\ref{app:e1-30b} conditions; not audited \\
\bottomrule
\end{tabular}
\end{footnotesize}
\end{center}

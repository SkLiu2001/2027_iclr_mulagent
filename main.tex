...
harness the data pipeline uses, and supervised fine-tuning (SFT) on that data teaches
...
The effect is measurable: fine-tuning \basemodel{} on \claudecode{}
...
with different tools and conventions (\openhands{}, \codexcli{}) degrades
...
every call into a semantic action. A vocabulary of $12$ actions suffices: after normalization
...
\item \textbf{A step-matched evaluation.} Against a control trained on the
  ...
  \method{} matches the control on the source harness, is ahead under all
  three unseen production harnesses on both benchmarks, and narrows the
  accuracy range across harnesses (Section~\ref{sec:step-matched}).
\end{itemize}
...
\end{proposition}
\noindent Appendix~\ref{app:proof} proves this by
...
$d(h,h') = \big|\{\, i : h_i \neq h'_i \,\}\big|$. We call a training method
that pursues this property harness-invariant, whence \method{}'s name.
...
instructive. \openhands{} exposes $4$ tools; \claudecode{} exposes $21$.
...
\method{} has two components, counterfactual harness rendering and interface anonymization, the second implemented as one tier of the first's name sampler;
...
property is a configuration parameter drawn from the same five axes. \cham{} parses calls, executes them against a real workspace, and formats
...
\paragraph{Name overlap with held-out harnesses.} \chr{}'s first
...
\paragraph{One base model, one source harness.} The main results use one
base model; an earlier run on \oldmodel{}
